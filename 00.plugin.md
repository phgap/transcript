...
这一节我们来实战演习一下，编写一个插件。这个插件的功能呢，是在编译时，将所有的资源打包成一个zip文件，可以在http请求时，在网络环境不是很顺畅的时候，减少下载的资源流量。

我们在前面讲过，compilation是处理整个编译流程的。那么我们就可以在资源生成之后，但是还没有生成的时候，将所有资源进行打包，然后将打包后的资源,再交给compilation进行管理，之后，在compilation的emitAsset阶段，统一由webpack将资源放到输出路径中。所以整理一下上面的需求，有
1. 插件接受参数，指定压缩后文件名
2. 将所有资源，按照生成到output的路径结构，压缩成一个zip文件
